...
- (void)initSubviews {
    ...
    for (int i = 0; i < _inputNum; i ++) {
        ...
        [CALayer addSubLayerWithFrame:CGRectMake(0, H-2, K_W, 2) backgroundColor:[ZHColor colorWithHex:@"#222227"] backView:subView];
        //Label
        UILabel *label = [[UILabel alloc]init];
        label.frame = CGRectMake(0, 0, K_W, H);
        label.textAlignment = NSTextAlignmentCenter;
        label.textColor = [ZHColor colorWithHex:@"#222227"];
        label.font = [ZHFont MediumMisansFontOfSize:26];
        [subView addSubview:label];
        //光标
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(K_W / 2, (H-25)/2.0, 3, H - 25)];
        CAShapeLayer *line = [CAShapeLayer layer];
        line.path = path.CGPath;
        line.fillColor =  [ZHColor colorWithHex:@"#2A82E4"].CGColor;
